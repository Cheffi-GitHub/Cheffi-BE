...
class MapValidationPolicyTest {

	@Spy
	private MapValidationPolicy mapValidationPolicy;
	@Spy
	...
	class ValidateTagsOfReview {
		...
		void parameterizedTest(List<Long> foodTagIds, List<Long> tasteTagIds, String flag, String message) {
			...
			if (flag.equals("fail"))
				assertThatExceptionOfType(BusinessException.class).isThrownBy(
					//when
					() -> mapValidationPolicy.validateTagsOfReview(REQUESTED_TAG_LIST, foodTagIds, tasteTagIds)
				);
			else
				//then
				assertThatNoException().isThrownBy(
